...
import { hexToRgb } from "app/core/components/dashboard/layout"
import NextChakraLink from "app/core/components/nextChakraLink"
import { shadow, themes } from "app/core/styles/theme"
...
export function parseText(text: any) {
  ...
  if (text.annotations.code) {
    ...
    const themeCookie = (themes[getCookie("theme")] && getCookie("theme")) || "default"
...
export function parseBlock(block: any): JSX.Element {
  switch (block.type) {
    ...
    case "code":
      return (
        <Box mt="10px" _hover={{ cursor: "text" }} style={{ tabSize: 2 }}>
          <CopyBlock
            text={block.code.text.map((b) => b.plain_text).join(", ")}
      ...
      return (
        <Box>
          <Image
            src={imageFile.url}
            mt="10px"
            alt={block.image?.caption?.[0]?.plain_text || "No alt text provided 😢"}
          />
        </Box>
      )
    case "callout":
      ...
      if (orderedList.length) {
        elementList.push(
          <OrderedList key={"block_" + cnt++} pl={6}>
            {orderedList.map((item, idx: number) => cloneElement(item, { key: "ol_" + idx }))}
          </OrderedList>
        )
        orderedList = []
      }
      ...
      if (unorderedList.length) {
        elementList.push(
          <UnorderedList key={"block_" + cnt++} pl={6}>
            {unorderedList.map((item, idx: number) => cloneElement(item, { key: "ul_" + idx }))}
          </UnorderedList>
        )
        unorderedList = []
      }
